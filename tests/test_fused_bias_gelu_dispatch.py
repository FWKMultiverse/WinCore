...
This only needs `torch` to be importable (_fused_bias_gelu_kernel.py imports
...
from WinCore.kernels._fused_bias_gelu_kernel import (
...
def test_default_threshold_used_when_no_env_var_and_not_calibrated(monkeypatch):
    ...
    import importlib; mod = importlib.import_module("WinCore.kernels._fused_bias_gelu_kernel")  # using importlib directly (not "import ... as mod") to get the actual submodule object -- WinCore.kernels._fused_bias_gelu_kernel, not the WinCore.kernels.fused_bias_gelu PUBLIC lazy attribute (a different name on purpose, see kernels/__init__.py's own docstring on why they must not collide)
