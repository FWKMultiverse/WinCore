...
def set_priority(level: str = "above_normal", pid: Optional[int] = None) -> str:
    ...
    *every* thread in the process (Python main loop, DataLoader worker
    ...
    try:
        ...
        raise PriorityError(
            "set_priority() needs psutil (pip install WinCore-Foundation[sysinfo], "
...
def cpu_vendor() -> str:
    """Best-effort CPU vendor: `"amd"`, `"intel"`, or `"unknown"`.

    Read via `platform.processor()` -- on Windows this is derived from
    the `PROCESSOR_IDENTIFIER` environment string (e.g. `"Intel64
    Family 6 Model 154 Stepping 3, GenuineIntel"` or `"AMD64 Family 25
    Model 33 Stepping 2, AuthenticAMD"`), which contains the actual
    CPUID vendor string (`GenuineIntel`/`AuthenticAMD`), not a guess
    derived from a model number. Never raises -- returns `"unknown"`
    for anything that doesn't match either (including non-Windows
    platforms, where `platform.processor()` often returns an empty
    string), which is the honest answer rather than a guess.
    """
    try:
        ident = (platform.processor() or "").lower()
    except Exception:
        return "unknown"
    if "authenticamd" in ident or "amd" in ident:
        return "amd"
    if "genuineintel" in ident or "intel" in ident:
        return "intel"
    return "unknown"


def numa_node_count() -> int:
    """Number of NUMA nodes Windows reports for this machine, via
    `GetNumaHighestNodeNumber`. Returns `1` (not an error -- "assume a
    single node") if this can't be determined: non-Windows, an OS
    build without the API, or any ctypes failure.

    Why this isn't Intel/AMD-specific: multi-socket Intel Xeon
    systems, and AMD Threadripper/EPYC (and some high-core-count Ryzen
    Threadripper) parts, can expose more than one NUMA node --
    crossing a NUMA boundary means a thread's memory accesses travel
    to a physically different memory controller, at real, measurable
    extra latency, regardless of vendor. This reads Windows' own
    topology report rather than special-casing one CPU brand.
    """
    if platform.system() != "Windows":
        return 1
    try:
        ...
        ok = ctypes.windll.kernel32.GetNumaHighestNodeNumber(ctypes.byref(highest))
        if not ok:
            return 1
...
def numa_node_cpus(node: int = 0) -> Optional[list]:
    """Logical CPU indices belonging to NUMA `node`, via
    `GetNumaNodeProcessorMaskEx` (Windows only). Returns `None` --
    never raises -- if this can't be determined: non-Windows, an OS
    build without the API, `node` doesn't exist, or any ctypes
    failure. `None` here means "couldn't determine it", the same
    "don't know, don't restrict anything" signal
    `_detect_windows_performance_cores()` uses for the P-core case.
    """
    if platform.system() != "Windows":
        return None
    try:
        ...
        ok = ctypes.windll.kernel32.GetNumaNodeProcessorMaskEx(
            wintypes.USHORT(node), ctypes.byref(affinity)
        )
        if not ok:
            return None
        cpus = []
...
def _select_pin_cpus(recommended: int, numa_aware: bool = True) -> list:
    ...
    `numa_aware` (default `True`): if this machine has more than one
    NUMA node (`numa_node_count() > 1` -- multi-socket Intel, or AMD
    Threadripper/EPYC-class parts, see `numa_node_count()`), narrow
    the candidate CPU pool to NUMA node 0's logical CPUs BEFORE
    applying P-core selection, so a pin doesn't spread a single
    process's threads across a NUMA boundary (which adds real,
    measurable remote-memory-access latency on every cross-node
    access) as a side effect of just picking "the first N indices".
    Node 0 is a conservative, deterministic default -- this function
    has no way to know which node's local memory your data actually
    lives in, so it doesn't guess further than "keep everything on
    one node instead of scattering across all of them". If node 0's
    CPU list can't be determined, or restricting to it would leave
    zero usable CPUs, this silently falls back to the non-NUMA-aware
    behavior rather than pinning to nothing.
    """
    candidate_cpus = None
    if numa_aware and numa_node_count() > 1:
        candidate_cpus = numa_node_cpus(0)

    p_cores = _detect_windows_performance_cores()
    if p_cores:
        pool = p_cores
        if candidate_cpus is not None:
            restricted = [c for c in pool if c in candidate_cpus]
            if restricted:
                pool = restricted
        # Use up to `recommended` of the (possibly NUMA-restricted)
        # P-core logical CPUs. If there are fewer than `recommended`,
        # use all of them rather than spilling onto E-cores/another
        # node -- under-using thread count is a smaller cost than
        # scheduling training compute onto efficiency cores or across
        # a NUMA boundary.
        return pool[:recommended] if len(pool) >= recommended else pool

    if candidate_cpus is not None:
        return candidate_cpus[:recommended] if len(candidate_cpus) >= recommended else candidate_cpus
...
def pin_affinity(cpus: Optional[list] = None, pid: Optional[int] = None) -> list:
    ...
    try:
        ...
        raise PriorityError(
            "pin_affinity() needs psutil (pip install WinCore-Foundation[sysinfo], "
            "or plain `pip install psutil`)."
        ) from exc
...
class ThreadPlan:
    ...
    recommended: int
    warnings: tuple = ()
    vendor: str = "unknown"  # see cpu_vendor() -- "amd" | "intel" | "unknown"; informational only, does not change sizing
...
def recommended_threads(
    ...
    if threads is not None:
        ...
        return ThreadPlan(total_logical=total, reserved=reserved, recommended=recommended, vendor=vendor)
...
def apply(
    ...
    reach (see module docstring):
      ...
      - `affinity`: if True, calls `pin_affinity()` with the same
        P-core-aware, NUMA-aware CPU selection `pin_affinity()` uses
        by default (`_select_pin_cpus()`, sized to `plan.recommended`
        -- this call's own thread count, which may differ from
      ...
      - `numa_aware`: only consulted when `affinity=True`. Default
        `True` restricts the affinity pin to NUMA node 0's CPUs on a
        multi-node machine (multi-socket Intel Xeon, AMD
        Threadripper/EPYC-class parts) before applying P-core
        selection, so a pin doesn't scatter threads across a NUMA
        boundary as a side effect. See `_select_pin_cpus()`'s own
        docstring for the exact precedence between NUMA restriction
        and P-core selection, and why node 0 specifically.
    ...
    if affinity:
        try:
            pin_affinity(_select_pin_cpus(plan.recommended, numa_aware=numa_aware))
        except PriorityError as exc:
            if strict:
                raise
            warnings.append(str(exc))
