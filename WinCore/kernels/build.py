...
def _check_cl() -> str:
    ...
    Returns the resolved path to `cl.exe` (not just `None` on success)
    -- `build()` feeds this straight to nvcc via `-ccbin=<path>`. This
    matters on its own, not just as a convenience: with very new MSVC
    toolsets (e.g. VS2026's v145, "18.x") released after a given CUDA
    Toolkit version, `nvcc` can fail its OWN internal host-compiler
    auto-detection even though `cl.exe` is genuinely present and
    correctly on PATH in the exact same shell -- `-allow-unsupported-
    compiler` only silences nvcc's version-mismatch WARNING, it does
    not fix nvcc failing to LOCATE the compiler at all, which surfaces
    as ninja's cryptic `CreateProcess failed: The system cannot find
    the file specified` instead of an actionable message. Passing
    `-ccbin` tells nvcc exactly where to look, skipping its own
    (potentially toolset-version-unaware) auto-detection entirely.
    """
    if os.name != "nt":
        return ""
    if _autodetect_and_setup_msvc():
        ...
        if cl_path:
            return cl_path
        # _autodetect_and_setup_msvc() returned True (so `cl` WAS
        # findable at the moment it checked), but shutil.which("cl")
        # came back empty just now -- exceptionally unlikely (PATH
        # changing mid-process), but rather than silently building
        # without -ccbin in that edge case, fall through to the same
...
def build(clean: bool = True):
    ...
    # BUGFIX (found via a real Windows + VS2026 + CUDA 13.x test run):
    # `_check_cl()` has ALWAYS resolved and returned the real path to
    # `cl.exe` -- its own docstring already said as much ("`build()`
    # feeds this straight to nvcc via `-ccbin=<path>`") -- but this
    # function never actually DID that: `cl_path` used to be discarded
    # (`_check_cl()` called with no assignment) and `extra_cuda_cflags`
    # never included `-ccbin` at all. That's harmless on a CUDA Toolkit
    # release new enough to already recognize the installed MSVC
    # toolset version -- nvcc's own auto-detection finds `cl.exe` fine
    # on its own there. But with a VERY new MSVC toolset (e.g. VS2026's
    # "18.x"/v145) released after the installed CUDA Toolkit version,
    # nvcc's internal host-compiler auto-detection can fail to locate
    # `cl.exe` even though it's genuinely present and correctly on
    # PATH in the exact same shell -- `-allow-unsupported-compiler`
    # (already passed below) only silences nvcc's version-MISMATCH
    # warning, it does not fix nvcc failing to find the compiler
    # EXECUTABLE at all. That failure doesn't surface as a clear nvcc
    # error either -- it comes out of ninja as a bare `CreateProcess
    # failed: The system cannot find the file specified`, pointing at
    # nvcc's own command line with no indication `cl.exe` was the
    # missing piece. Passing `-ccbin <cl_path>` tells nvcc exactly
    # where to look, skipping its own (potentially toolset-version-
    # unaware) auto-detection entirely -- confirmed to resolve this
    # exact failure on a real VS2026 Dev Command Prompt + CUDA 13.3
    # machine where `nvcc -ccbin "<cl.exe path>"` succeeded on a
    # trivial .cu file after the same invocation without `-ccbin`
    # failed with precisely this error.
    extra_cuda_cflags = ["-O3", "-allow-unsupported-compiler"]
    if cl_path:
        extra_cuda_cflags += ["-ccbin", cl_path]
