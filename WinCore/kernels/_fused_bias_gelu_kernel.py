...
Why this file is named `_fused_bias_gelu_kernel.py`, not `fused_bias_gelu.py`
--------------------------------------------------------------------------------
The public function this module defines IS called `fused_bias_gelu`
(see below) -- but naming the FILE identically to that function caused
a real, confirmed bug (found via a real-hardware test run; see
CHANGELOG). Python's import system unavoidably binds a submodule onto
its parent package's namespace under the submodule's own name as a
side effect of ANY import of that submodule -- `import
WinCore.kernels.fused_bias_gelu`, `from WinCore.kernels.fused_bias_gelu
import X`, or even `WinCore.kernels`'s own internal lazy-loading
`importlib.import_module()` call all trigger this, and there was no
way to prevent it while the file and the function shared a name. Any
code anywhere (including, in practice, a test file that legitimately
needed a private helper not exposed through the lazy public API) that
directly imported the submodule would silently overwrite
`WinCore.kernels.fused_bias_gelu` -- the PUBLIC attribute -- from "the
function" to "the module object itself", breaking every subsequent
call to it with `TypeError: 'module' object is not callable`, for the
rest of that process. Renaming the FILE (not the function -- see
`kernels/__init__.py`'s `_LAZY_EXPORTS`, which still exposes this as
`WinCore.kernels.fused_bias_gelu`, unchanged) removes the naming
collision at its root: a direct import of THIS file
(`WinCore.kernels._fused_bias_gelu_kernel`) auto-binds a DIFFERENT
name than the public `fused_bias_gelu` attribute, so the two can never
collide again, regardless of how or in what order anything imports
either one.
