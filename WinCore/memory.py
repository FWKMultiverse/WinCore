...
class PinnedBufferPool:
    """Reuses pinned (page-locked) CPU tensors by `(shape, dtype)`
    instead of allocating and pinning a fresh buffer on every call.

    Why this matters: `torch.empty(shape, dtype=dtype, pin_memory=True)`
    is not a plain heap allocation -- pinning a page-locked host buffer
    goes through a CUDA driver call under the hood
    (`cudaHostAlloc`/`cudaHostRegister`), which carries real,
    measurable per-call overhead well above an ordinary allocation. A
    staging pipeline that allocates a same-shaped pinned buffer every
    step (e.g. a custom collate/transfer path outside `DataLoader`'s
    own `pin_memory=True`, which already pools internally) pays that
    overhead every single step for no reason -- the buffer's
    *contents* change every step, but its shape/dtype usually doesn't.

    This keeps a small, bounded dict of already-pinned buffers keyed
    by `(shape, dtype)` and hands the same underlying storage back on
    the next `get()` for that shape instead of pinning again.

    Explicitly NOT automatic
    ---------------------------
    - The caller must overwrite a reused buffer's contents before
      using it (`buf.copy_(new_data)`, or write into it directly) --
      this pool does not zero or clear a reused buffer on `get()`,
      since doing that would cost exactly the write bandwidth reuse is
      meant to save.
    - The caller must call `release(tensor)` when done with a buffer
      for it to be reusable later -- there is no automatic
      reference-counting or `__del__`-based return here (that would
      require the caller to give up their only reference at exactly
      the right time, which this pool has no way to enforce or detect
      reliably).
    - NOT thread-safe by design, same as this module's other
      single-consumer helpers (see `CacheGuard`) -- if multiple
      threads share one pool, guard access with your own lock. Each
      DataLoader worker *process* on Windows gets its own independent
      interpreter (see this module's own docstring on `spawn`), so a
      pool constructed per-worker needs no cross-process
      synchronization at all -- only a same-process, multi-thread use
      would need one.
    ...
    Example:
        ...
        buf.copy_(batch)
        gpu_tensor = to_device_non_blocking(buf, "cuda:0")
        # ... use gpu_tensor ...
    ...
    def get(self, shape, dtype=None):
        """Return a pinned CPU tensor of `shape`/`dtype`: a previously
        `release()`-d buffer of the same `(shape, dtype)` if the pool
        has one, otherwise a freshly allocated-and-pinned
        `torch.empty(shape, dtype=dtype, pin_memory=True)`. Contents
        are UNDEFINED either way (stale prior data if reused,
        uninitialized memory if freshly allocated -- same as
        `torch.empty` itself promises) -- always overwrite before use.
        """
        import torch
    ...
    def release(self, tensor) -> None:
        """Return `tensor` to the pool so a future `get()` with the
        same shape/dtype reuses it, instead of letting it be garbage-
        collected (which frees the pinned allocation -- the exact cost
        this pool exists to avoid paying again next time).

        If adding this buffer pushes the pool past `max_buffers`
        total (across every shape/dtype combination, not just this
        one), the least-recently-released buffer is evicted first --
        a bounded pool, so a training loop that legitimately touches
        many distinct shapes over time (e.g. variable-length sequence
        batches) can't turn this into an unbounded pinned-memory leak.
        """
        key = (tuple(tensor.shape), tensor.dtype)
        self._pool[key] = tensor
        self._pool.move_to_end(key)
        while len(self._pool) > self.max_buffers:
