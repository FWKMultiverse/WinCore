...
@dataclass
class WarmReport:
    """Returned by `DiskCache.warm()`."""

    requested: int
    already_cached: int
    computed: int
    failed: int
    errors: list  # list of (key, Exception) tuples, empty if failed == 0
...
class DiskCache:
    ...
    def _store(self, filename: str, path: Path, value: Any) -> None:
        self._store_no_evict(filename, path, value)
        self._evict_if_needed()

    def _store_no_evict(self, filename: str, path: Path, value: Any) -> None:
        """Same write + index bookkeeping as `_store()`, WITHOUT the
        trailing `_evict_if_needed()` call. Exists solely so `warm()`
        can write many entries without paying `_evict_if_needed()`'s
        full-directory-rescan-under-a-cross-process-lock cost once PER
        KEY -- see `warm()`'s own docstring for why that matters. Not
        part of the public API; `get_or_compute()` still goes through
        `_store()` (this plus one eviction pass per call), unchanged.
        """
    ...
    def warm(
        ...
        """Pre-populate cache entries for `keys` before a training loop
        needs them, computing multiple entries CONCURRENTLY instead of
        one at a time through repeated `get_or_compute()` calls.

        Why concurrent computation helps here, specifically
        -----------------------------------------------------
        This isn't premised on "SSDs are fast at random writes" alone
        (true, but the write itself is usually the cheap part of a
        cache-miss). Most real `compute_fn`s spend most of their time
        in C-extension work that releases Python's GIL -- image
        decode/resize (PIL/OpenCV/torchvision), NumPy array ops,
        tokenization -- with the actual file write at the end being
        comparatively fast. Running several `compute_fn()` calls
        concurrently via a thread pool overlaps that CPU-bound decode/
        resize work across threads (real parallelism for GIL-releasing
        C extensions) AND overlaps each entry's disk write with the
        next entry's compute, instead of paying decode-then-write,
        decode-then-write serially. An SSD's own multiple internal I/O
        queues (unlike a spinning disk, where concurrent writers can
        make things WORSE by turning sequential writes into seeks)
        mean the write side doesn't become a new bottleneck once
        several are in flight -- this doesn't verify the drive is
        actually an SSD any more than the rest of this module does
        (see the module docstring).

        If your `compute_fn` is pure-Python CPU-bound work that never
        releases the GIL, threads won't parallelize that part -- this
        method still overlaps I/O with the next item's Python-level
        work, but won't give you multi-core speedup on the compute
        itself; that would need a process pool, which this method
        deliberately does not use (see below).

        Why threads, not processes
        -----------------------------
        A `ProcessPoolExecutor` would need `compute_fn` (and everything
        it closes over) to be picklable, and on Windows specifically
        would re-import your script's module-level code per worker
        process -- the same `spawn` cost this package's `memory`
        module already documents for `DataLoader` workers. A thread
        pool avoids both: no pickling requirement, no process startup
        cost, and it's still real parallelism for the GIL-releasing
        C-extension work most `compute_fn`s are actually bottlenecked
        on.

        Already-cached keys are skipped without calling `compute_fn`
        (same `path.exists()` check `get_or_compute()` uses) -- calling
        `warm()` again on a partially-warmed cache (e.g. resuming after
        an interrupted warm pass, or a second dataset that shares some
        keys with the first) only computes what's still missing.

        Eviction runs ONCE, after every key has been stored -- not per
        key. `get_or_compute()`'s normal path runs `_evict_if_needed()`
        (a full directory rescan under a cross-process lock -- see that
        method's docstring) after every single store, which is correct
        for one-at-a-time calls but would defeat the point of THIS
        method if repeated per key here: `warm()`'s whole value is
        letting several `compute_fn()` calls run concurrently, and
        serializing every one of them behind a full-directory-rescan-
        under-a-lock on every write would flatten that concurrency
        right back out, especially with many workers writing many
        small entries. Deferring eviction to a single pass at the end
        keeps the per-key path lock-free (only the final index update
        takes the in-process lock, briefly) and still leaves the cache
        under `max_bytes` by the time `warm()` returns.

        Args:
            keys: iterable of cache keys (consumed once into a list --
                a generator is fine).
            compute_fn: called as `compute_fn(key)` for each miss.
                Note this takes `key` as an argument, unlike
                `get_or_compute`'s zero-argument closure -- `warm()` is
                driving many keys itself, so it needs to tell
                `compute_fn` which one each call is for.
            max_workers: forwarded to `ThreadPoolExecutor`. `None`
                (default) uses that class's own default (based on
                `os.cpu_count()`).
            on_error: called as `on_error(key, exception)` if
                `compute_fn` raises for a given key. Warming continues
                for the remaining keys either way -- one bad sample
                does not abort the whole warm pass. `None` (default,
                no callback) still records the failure in the returned
                `WarmReport` -- errors are never silently dropped,
                only NOT re-raised out of this call; check
                `report.errors` / `report.failed`.

        Returns a `WarmReport` with counts of what was newly computed
        vs. already-cached vs. failed, plus the actual `(key,
        exception)` pairs for anything that failed.
        """
        ...
        if to_compute:
            ...
            def _do_one(key):
                ...
                # _store_no_evict, not _store -- see the docstring's
                # "Eviction runs ONCE" section. Eviction for this whole
                # warm() call happens exactly once, below, after every
                # key has been written.
                self._store_no_evict(filename, path, value)

            with ThreadPoolExecutor(max_workers=max_workers) as pool:
                futures = {pool.submit(_do_one, key): key for key in to_compute}
            ...
            # One eviction pass for the whole batch, not one per key --
            # see the docstring. Runs even if every key failed (a no-op
            # rescan in that case) so a `warm()` call always leaves the
            # cache at a consistent, budget-respecting state on return,
            # same guarantee `get_or_compute()` gives per call.
            self._evict_if_needed()
    ...
    def _evict_if_needed(self) -> None:
        ...
        try:
            with _CrossProcessLock(self._lock_path, timeout=self._lock_timeout):
                ...
                # BUGFIX (v0.8.2, was #8.4): rebuilding the index via
                # entries[i:] assumed `i` only ever advances past
                # entries that were *evicted*. But on an unlink()
                # failure (antivirus/indexer holding the file, a
                # permissions blip) the old code also did `i += 1`
                # before `continue` -- so that entry's index slot was
                # skipped too, even though the file is still sitting on
                # disk. Net effect: `self._index` silently forgot about
                # a file that's still there, so `__len__()` undercounts
                # (stats.bytes_on_disk was unaffected since `total`
                # was correctly left unchanged on failure). Track
                # evicted names explicitly instead of relying on `i`'s
                # final position, so a failed unlink just leaves that
                # entry exactly where it was -- present on disk AND
