...
Also re-exported directly: `atomic_write`, `AtomicWriteError`, `atomic_torch_save`, `atomic_safetensors_save`, `safe_compile`, `should_compile`, `SafeCompiled` — everything else lives under its module (`WinCore.spec`, `WinCore.cpu`, etc.), shown below.
...
- if `affinity=True`, calls `pin_affinity(cpus)` where `cpus` comes from the same P-core-aware selection `pin_affinity()` uses by default (`_select_pin_cpus()`, sized to `plan.recommended`) — not a bare `range(plan.recommended)`
...
Pins the process to specific logical CPUs (`SetProcessAffinityMask` / `sched_setaffinity`, via `psutil`). `cpus` defaults to a P-core-aware selection on Windows (`GetLogicalProcessorInformationEx`-based; falls back to `range(recommended_threads().recommended)` when core-type info isn't available). Not supported on macOS (no public affinity API there) — raises `PriorityError` in that case, same as any other OS/permission denial.
